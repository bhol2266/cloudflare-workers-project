...
// .wrangler/tmp/bundle-kr7Yz0/checked-fetch.js
...
// .wrangler/tmp/bundle-kr7Yz0/middleware-loader.entry.ts
init_checked_fetch();
init_modules_watch_stub();

// .wrangler/tmp/bundle-kr7Yz0/middleware-insertion-facade.js
init_checked_fetch();
init_modules_watch_stub();
...
// src/chutlunds/getHomePageVideos.js
init_checked_fetch();
init_modules_watch_stub();
async function getHomePageVideos(request) {
  if (request.method !== "POST") {
    return new Response(JSON.stringify({ message: "Only POST requests are allowed" }), {
      status: 405,
      headers: { "Content-Type": "application/json" }
  ...
  try {
    ...
    const homepageVideos = [
      { URL: "https://spankbang.party/trending_videos/", videosGroupName: "Trending" },
...
// src/chutlunds/getChannelVideos.js
init_checked_fetch();
init_modules_watch_stub();
async function getChannelVideos(request) {
  if (request.method !== "POST") {
    return new Response(JSON.stringify({ message: "Only POST requests are allowed" }), {
      status: 405,
      headers: { "Content-Type": "application/json" }
...
// src/chutlunds/getTrendingChannels.js
init_checked_fetch();
init_modules_watch_stub();
async function getTrendingChannels(request) {
  try {
    const requestBody = await request.json();
    let url = requestBody.url;
    if (url.includes("https://spankbang.com/")) {
...
// src/chutlunds/getTrendingSearchTags.js
init_checked_fetch();
init_modules_watch_stub();
async function getTrendingSearchTags(request) {
  if (request.method !== "POST") {
    return new Response(JSON.stringify({ message: "Only POST requests are allowed" }), {
      status: 405,
      headers: { "Content-Type": "application/json" }
...
// src/chutlunds/getPornstarVideos.js
init_checked_fetch();
init_modules_watch_stub();
async function getPornstarVideos(request) {
  if (request.method !== "POST") {
    return new Response(JSON.stringify({ message: "Only POST requests are allowed" }), {
      status: 405,
      headers: { "Content-Type": "application/json" }
  ...
  try {
    ...
    $2(".relative a").each((i, el) => {
      const name2 = $2(el).find("p").text().trim();
      ...
      if (name2 && image) {
        similarProfiles.push({ name: name2, image, link });
      }
    });
...
// src/chutlunds/getCreatorsPage.js
init_checked_fetch();
init_modules_watch_stub();
async function getCreatorsPage(request) {
  try {
    const requestBody = await request.json();
    let url = requestBody.url;
    const response = await fetch(url);
    const html3 = await response.text();
    const $2 = load(html3);
    const creatorName = $2("h1.text-primary").text().trim();
    var creatorImage = $2('img[alt="' + creatorName + '"]').attr("src");
    if (creatorImage) {
      creatorImage = creatorImage.replace(/^\/\//, "https://").replace(".com", ".party");
    }
    const creatorHref = $2(".grid.grid-cols-1.gap-2.md\\:mt-2.md\\:grid-cols-2 a").attr("href");
    const videosCount = $2('span:contains("Videos") em').text().trim();
    const viewsCount = $2('span:contains("Views") em').text().trim();
    const subscribersCount = $2('span:contains("Subscribers") em').text().trim();
    const description = $2("p.text-body-md").text().trim();
    const creatorData = {
      creatorName,
      creatorImage,
      creatorHref,
...
// src/desikahaniya/index.js
init_checked_fetch();
init_modules_watch_stub();

// src/desikahaniya/getVideos.js
init_checked_fetch();
init_modules_watch_stub();
...
// src/desikahaniya/index.js
var desikahaniya_default = {
  async fetch(request) {
    const url = new URL(request.url);
    const path = url.pathname;
    if (path === "/desikahaniya/getVideos") {
      return await getVideos2();
    }
  ...
  async fetch(request) {
    ...
    try {
      if (path.startsWith("/chutlunds")) {
        return await chutlunds_default.fetch(request);
      } else if (path.startsWith("/desikahaniya")) {
        return await desikahaniya_default.fetch(request);
      }
      return new Response("Not Found", {
        status: 404,
        headers: { "Content-Type": "application/json" }
